...
public class EuGameClass extends DPSSPOA {
    ...
    public String createPlayerAccount(String firstName, String lastName, String age, String userName, String password, String ipAddress){
        ...
        try {
            fileWriter = new FileWriter("src/replica3/eu/euAccounts.txt",true);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        try {
            if(!util.checkNameExist("src/replica3/eu/euAccounts.txt",str.split("\\|")[0])){
        ...
        File file = new File("src/replica3/logger/logs/euPlayerLogs/"+userName+".txt");
        if(!file.exists()){
            try {
                file.createNewFile();
    ...
    public String getPlayerStatus(String adminUsername, String adminPassword, String ipAddress){
        ...
        try {
            ...
            asMsg = util.getRemote(PortConfig.replicaAS3,aSocket,"");
    ...
    public boolean addPlayer(String str, String newIPAddress){
        ...
        if(newIPAddress.startsWith("132.")){
            ...
            try {
                aSocket1 = new DatagramSocket();
                asMsg = util.getRemote(PortConfig.replicaAS3,aSocket1,str);
            } catch (SocketException e) {
                System.out.println("Socket: " + e.getMessage());
            } catch (IOException e) {
                e.printStackTrace();
