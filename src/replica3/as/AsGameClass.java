...
public class AsGameClass extends DPSSPOA {
    ...
    public String createPlayerAccount(String firstName, String lastName, String age, String userName, String password, String ipAddress) {
        ...
        try {
            fileWriter = new FileWriter("src/replica3/as/asAccounts.txt",true);
        } catch (IOException ex) {
            ex.printStackTrace();
        }
        ...
        try {
            if(!util.checkNameExist("src/replica3/as/asAccounts.txt",str.split("\\|")[0])){
        ...
        File file = new File("src/replica3/logger/logs/asPlayerLogs/"+userName+".txt");
        if(!file.exists()){
            try {
                file.createNewFile();
    ...
    public String getPlayerStatus(String adminUsername, String adminPassword, String ipAddress){
        ...
        try {
            ...
            naMsg = util.getRemote(PortConfig.replicaNA3,aSocket1,"");
        } catch (SocketException e) {
            System.out.println("Socket: " + e.getMessage());
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public boolean addPlayer(String str, String newIPAddress){
        ...
        if(newIPAddress.startsWith("132.")){
            try {
                aSocket = new DatagramSocket();
                naMsg = util.getRemote(PortConfig.replicaNA3,aSocket,str);
            } catch (SocketException e) {
                System.out.println("Socket: " + e.getMessage());
            } catch (IOException e) {
                e.printStackTrace();
