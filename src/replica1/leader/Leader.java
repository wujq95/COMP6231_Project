...
public class Leader {
    ...
    public static void listenBroadCast() throws Exception{
        ...
        try{
            ...
            while(true){
                ...
                System.out.println("Leader1 receives a message from broadcast: "+requestData.trim());
                String result = operation(requestData);
                byte[] sendData = result.getBytes();
                DatagramPacket reply = new DatagramPacket(sendData, result.length(), request.getAddress(),
                        request.getPort());
                System.out.println("Leader1 replies to broadcast: "+result.trim());
                aSocket.send(reply);
            }
        }catch (SocketException e) {
            System.out.println("SocketException: " + e.getMessage());
    ...
    public static void notifyRM(String Msg,Integer port){
        ...
        try {
            ...
            System.out.println("Leader1 notify a failure to replica manager: "+Msg.trim());
            aSocket.send(request);
            while(true){
                try {
                    byte[] buffer = new byte[1000];
