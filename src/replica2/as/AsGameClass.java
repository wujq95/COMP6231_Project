...
import replica2.DPSSModule.DPSSPOA;
import replica2.logger.Logger;
import replica2.util.Util;
...
public class AsGameClass extends DPSSPOA {
    ...
    public String createPlayerAccount(String firstName, String lastName, String age, String userName, String password, String ipAddress) {
        ...
        try {
            fileWriter = new FileWriter("src/replica2/as/asAccounts.txt",true);
        } catch (IOException ex) {
            ex.printStackTrace();
        }
        ...
        try {
            if(!util.checkNameExist("src/replica2/as/asAccounts.txt",str.split("\\|")[0])){
        ...
        File file = new File("src/replica2/logger/logs/asPlayerLogs/"+userName+".txt");
        if(!file.exists()){
            try {
                file.createNewFile();
    ...
    public String playerSignIn(String userName, String password, String ipAddress){
        ...
        AsGameServer.asiaOnline.add(userName.trim());
    ...
    public String getPlayerStatus(String adminUsername, String adminPassword, String ipAddress){
        ...
        int sum = 0;
        for (Map.Entry<Character, List<String>> entry : AsGameServer.asiaMap.entrySet()) {
            sum += entry.getValue().size();
        }
        int online = AsGameServer.asiaOnline.size();
        ...
        try {
            ...
            euMsg = util.getRemote(PortConfig.replicaEU2,aSocket,"");
        ...
        try {
            ...
            naMsg = util.getRemote(PortConfig.replicaNA2,aSocket1,"");
        } catch (SocketException e) {
            System.out.println("Socket: " + e.getMessage());
        } catch (IOException e) {
            e.printStackTrace();
        ...
        String res = "NA: "+ naMsg.split(";")[0]+" online, "+naMsg.split(";")[1]+"offline, "+
                ...
                "AS: "+online+"online, "+(sum-online)+" offline";
        try {
    ...
    public boolean addPlayer(String str, String newIPAddress){
        ...
        if(newIPAddress.startsWith("132.")){
            try {
                aSocket = new DatagramSocket();
                naMsg = util.getRemote(PortConfig.replicaNA2,aSocket,str);
            } catch (SocketException e) {
                System.out.println("Socket: " + e.getMessage());
            } catch (IOException e) {
                e.printStackTrace();
            ...
            try {
                aSocket1 = new DatagramSocket();
                euMsg = util.getRemote(PortConfig.replicaEU2,aSocket1,str);
            } catch (SocketException e) {
                System.out.println("Socket: " + e.getMessage());
            } catch (IOException e) {
                e.printStackTrace();
