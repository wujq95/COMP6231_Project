...
public class NaGameClass extends DPSSPOA {
    ...
    public String createPlayerAccount(String firstName, String lastName, String age, String userName, String password, String ipAddress){
        ...
        try {
            fileWriter = new FileWriter("src/replica2/na/naAccounts.txt",true);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        try {
            if(!util.checkNameExist("src/replica2/na/naAccounts.txt",str.split("\\|")[0])){
        ...
        File file = new File("src/replica2/logger/logs/naPlayerLogs/"+userName+".txt");
        if(!file.exists()){
            try {
                file.createNewFile();
    ...
    public String getPlayerStatus(String adminUsername, String adminPassword, String ipAddress)  {
        ...
        try {
            ...
            asMsg = util.getRemote(PortConfig.replicaAS2,aSocket1,"");
        } catch (SocketException e) {
            System.out.println("Socket: " + e.getMessage());
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public boolean addPlayer(String str, String newIPAddress){
        ...
        if(newIPAddress.startsWith("182.")){
            try {
                aSocket = new DatagramSocket();
                asMsg = util.getRemote(PortConfig.replicaAS2,aSocket,str);
            } catch (SocketException e) {
                System.out.println("Socket: " + e.getMessage());
            } catch (IOException e) {
                e.printStackTrace();
